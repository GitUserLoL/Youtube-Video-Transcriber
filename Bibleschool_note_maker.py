...
def generate_commentary(transcript: str, title: str, max_retries: int = 3) -> Optional[str]:
    """Generate biblical commentary using Gemini API with retry logic."""
    if genai is None:
        return None
    
    prompt = '''You are an academic Bible commentator. You will receive a raw sermon/teaching transcript.

GOAL
Produce thorough, study-note–ready commentary that is strictly faithful to the transcript and to Scripture.

TRANSLATION & SOURCES
- Quote Bible verses from **KJV only**, verbatim with references (e.g., **KJV – Heb 3:1–2**).
- Use Scripture to interpret Scripture. Do not add outside authorities unless the transcript mentions them.

FIDELITY
- Keep every Scripture quotation and every exegetical point the speaker makes (definitions, claims, cross-references).
- Do not invent or “smooth over” gaps. If unclear, mark **[Ambiguous in transcript]**.
- Tag any cross-reference you add (not cited by the speaker) as **[Model-added cross-ref]**.
- Tag any brief lexical note you add (not raised by the speaker) as **[Model-added lexical]**.

OMIT
- Stories, jokes, greetings, announcements, prayers, or anecdotes not explaining the passage.

WORKFLOW (before writing)
1) **Extract** all Scripture citations from the transcript (map each to KJV text).
2) **Map** each substantial claim to the verse(s) it explains.
3) **Deduplicate** overlaps; preserve distinct arguments with their verse anchors.
4) **Quality check**: KJV verbatim, logical verse order, every claim traceable to a verse.

REQUIRED HEADINGS (use exactly these)
### Header
- **Passage/Theme:**
- **Speaker’s Main Thesis (1–2 sentences):**
- **Key Terms (brief glossary):** (term — definition)

### Historical/Canonical Context (only if in transcript)
- Authorship/Date/Audience as stated or implied by the speaker (note uncertainty if any).

### Verse-by-Verse (or Theme) Commentary
For each verse/theme block:
- **Heading**: e.g., “Heb 3:1–3 — Jesus the Apostle and High Priest”
- **Scripture (KJV)**: block-quote or bold with reference.
- **Commentary**: dense bullets that track the speaker’s logic; tie each bullet to the verse.
- **Cross-References (KJV)**: list those the speaker cites; tag additions **[Model-added cross-ref]**.

### Theological Threads (from transcript)
- Bullets for arguments that span multiple verses (e.g., endurance, priesthood, covenant).

### Original Language Notes (concise)
- Only where the speaker focuses on a word; otherwise add sparingly and tag **[Model-added lexical]**.

### Disputed / Alternate Views (from transcript)
- Bullets summarizing views the speaker mentions and why.

### Not Covered by Speaker (if applicable)
- List verses/sections in the range the speaker skipped, with a one-line reason based on the transcript.

ASSERTION GUARD
- Avoid absolute claims (“first/always/never”) unless the transcript states them. Prefer “uniquely called in Heb 3:1,” “appears to,” or mark **[Ambiguous in transcript]**.

FORMATTING
- Scripture in block quotes or bold; commentary in concise bullets/short paragraphs.
- Keep prose tight; no filler.

CONSTRAINTS
- KJV only for quotations (verbatim).
- No application/illustrations unless they directly explain the text.

SELF-CHECK (silently before finalizing)
- KJV verbatim for each quoted verse?
- All required headings present?
- All model-added cross-refs/lexical notes tagged?
- No unsubstantiated absolutes (or marked **[Ambiguous in transcript]**)?

'''+transcript+'''

OUTPUT:
A polished, well-structured commentary using the headings above, thorough and faithful to the pastor’s notes, with rich verse-by-verse detail.
'''
    ...
    for attempt in range(max_retries):
        try:
            ...
            if attempt > 0:
                ...
                print(f"🤖 Generating biblical commentary with {model_str}...")
...
def process_playlist(playlist_url: str, transcript_dir: Path, commentary_dir: Path, use_gemini: bool = True) -> None:
    ...
    try:
        ...
        for i, video_url in enumerate(videos, 1):
            ...
            # Add 20 second delay to avoid rate limiting
